...
De forma predeterminada, la configuración de Dispatcher se almacena en el archivo de texto `dispatcher.any`, aunque, durante la instalación, puede cambiar el nombre y la ubicación de este archivo.
...
## Asignación de un nombre a la instancia de Dispatcher {#naming-the-dispatcher-instance-name}
...
La propiedad `/clientheaders` define una lista de encabezados HTTP que Dispatcher pasa de la petición HTTP del cliente al procesador (instancia de AEM).
...
## Identificar los hosts virtuales {#identifying-virtual-hosts-virtualhosts}
...
#### La parte de línea de solicitud de las solicitudes HTTP {#the-request-line-part-of-http-requests}
...
#### Comillas dobles frente a comillas simples {#double-quotes-vs-single-quotes}
...
Si los filtros no se activan del modo esperado, habilite el [Registro de rastreo](#trace-logging) en Dispatcher para que pueda ver qué filtro está interceptando la solicitud.
...
Las solicitudes a un área denegada explícitamente dan como resultado el código de error 404 (página no encontrada).
...
>
>En el ejemplo anterior, si las solicitudes a `/etc` que no tienen una cadena de consulta deben ser permitidas también, se requieren las siguientes reglas:
>
...
Al añadir `/loadOnStartup 0` (ver el ejemplo a continuación), puede deshabilitar la carga de las URL mnemónicas durante el inicio.
...
Mientras `/loadOnStartup 1` carga las direcciones URL mnemónicas durante el inicio, tenga en cuenta que `/loadOnStartup 1` es el valor predeterminado actual para este parámetro.
...
### Nombre del archivo de estado {#naming-the-statfile}
...
### Servir documentos obsoletos cuando se producen errores {#serving-stale-documents-when-errors-occur}
...
### Almacenamiento en la caché cuando se utiliza la autenticación {#caching-when-authentication-is-used}
...
### Ignorar los parámetros de URL {#ignoring-url-parameters}
...
>[!NOTE]
>
>Al configurar la propiedad glob, debe coincidir con el nombre del parámetro de consulta. Por ejemplo, si desea ignorar el parámetro &quot;p1&quot; de la siguiente URL `http://example.com/path/test.html?p1=test&p2=v2`, la propiedad glob debe ser:
...
### Almacenamiento en caché de los encabezados de respuesta HTTP {#caching-http-response-headers}
...
### Limitación del archivo .stat tocándolo {#throttling-stat-file-touching}
...
Estos mensajes se pueden generar cuando `EINTR` ocurre en la sección `read more data`. El problema se produce porque una señal llega antes que cualquier dato.
...
* **no almacenable en caché: la solicitud debía ser un GET o HEAD**
El método HTTP no es GET ni HEAD. Dispatcher supone que la salida contiene datos dinámicos que no deben almacenarse en caché.
