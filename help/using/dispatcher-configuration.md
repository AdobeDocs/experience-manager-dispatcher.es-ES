...
AEM De forma predeterminada, Dispatcher reenvía los encabezados HTTP estándar a la instancia de. En algunos casos, es posible que desee reenviar encabezados adicionales o eliminar encabezados específicos:
...
2. Si el archivo no ha caducado o `/enableTTL` no está configurado, se aplican las reglas de invalidación de caché estándar como las reglas que [`/statfileslevel`](#invalidating-files-by-folder-level) y [`/invalidate`](#automatically-invalidating-cached-files) set. Este flujo significa que Dispatcher puede invalidar archivos para los que el TTL no ha caducado.
